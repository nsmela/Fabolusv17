...
namespace Fabolus.Features.AirChannel {
    ...
    public class AirChannelPath : AirChannelShape {
        ...
        private void SetGeometry() {
            ...
            AddSphere(0, ref mesh);
            AddCylinder(0, ref mesh);
            for (int i = 1; i < _path.Count; i++) {
                //channel to previous point
                AddSphere(i, ref mesh);
                AddCylinder(i, ref mesh);
            }
            Geometry = mesh.ToMesh();
        }

        private void AddSphere(int pointIndex, ref MeshBuilder mesh) => mesh.AddSphere(Path(pointIndex), _radius);
        private void AddCylinder(int pointIndex, ref MeshBuilder mesh) => mesh.AddCylinder(Path(pointIndex), TopPoint(Path(pointIndex)), _radius, 16, true, true);
